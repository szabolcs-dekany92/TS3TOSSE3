...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"TS3toSSEngine3";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "TS3toSSEngine3";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
	return "TS3toSSEngine3";
#endif
}
...
int ts3plugin_init() {
    ...
    /*
	 * Methods regarding the initialization of SSE3 specific settings
...
int ts3plugin_offersConfigure() {
	printf("TS3TOSSE3: offersConfigure\n");
	/*
	 * Return values:
...
void ts3plugin_registerPluginID(const char* id) {
	...
	printf("TS3TOSSE3: registerPluginID: %s\n", pluginID);
}
...
int ts3plugin_processCommand(uint64 serverConnectionHandlerID, const char* command) {
	...
	printf("TS3TOSSE3: process command: '%s'\n", command);
		...
		case CMD_COMMAND:  /* /test command <command> */
			if(param1) {
				...
				if(pluginID) {
					...
					printf("TS3TOSSE3: Sending plugin command to current channel: %s\n", param1);
					ts3Functions.sendPluginCommand(serverConnectionHandlerID, pluginID, param1, PluginCommandTarget_CURRENT_CHANNEL, NULL, NULL);
				} else {
					printf("TS3TOSSE3: Failed to send plugin command, was not registered.\n");
				}
			} else {
...
void ts3plugin_currentServerConnectionChanged(uint64 serverConnectionHandlerID) {
    printf("TS3TOSSE3: currentServerConnectionChanged %llu (%llu)\n", (long long unsigned int)serverConnectionHandlerID, (long long unsigned int)ts3Functions.getCurrentServerConnectionHandlerID());
}
...
void ts3plugin_onConnectStatusChangeEvent(uint64 serverConnectionHandlerID, int newStatus, unsigned int errorNumber) {
    ...
    if(newStatus == STATUS_CONNECTION_ESTABLISHED) {  /* connection established and we have client and channels available */
        ...
        if(ts3Functions.getClientLibVersion(&s) == ERROR_ok) {
            printf("TS3TOSSE3: Client lib version: %s\n", s);
        ...
        printf("TS3TOSSE3: Server name: %s\n", s);
        ...
        printf("TS3TOSSE3: Server welcome message: %s\n", s);
        ts3Functions.freeMemory(s);  /* Release string */
        ...
        printf("TS3TOSSE3: My client ID = %d, nickname = %s\n", myID, s);
        ts3Functions.freeMemory(s);
        ...
        printf("TS3TOSSE3: Available channels:\n");
        for(i=0; ids[i]; i++) {
            /* Query channel name */
...
int ts3plugin_onTextMessageEvent(uint64 serverConnectionHandlerID, anyID targetMode, anyID toID, anyID fromID, const char* fromName, const char* fromUniqueIdentifier, const char* message, int ffIgnored) {
    printf("TS3TOSSE3: onTextMessageEvent %llu %d %d %s %s %d\n", (long long unsigned int)serverConnectionHandlerID, targetMode, fromID, fromName, message, ffIgnored);
...
int ts3plugin_onClientPokeEvent(uint64 serverConnectionHandlerID, anyID fromClientID, const char* pokerName, const char* pokerUniqueIdentity, const char* message, int ffIgnored) {
	...
	struct PokeParameters *tPokeParameters = malloc(sizeof(struct PokeParameters));
	tPokeParameters->user = stringcopywithpointer(pokerName);
	tPokeParameters->messages = stringcopywithpointer(message);

	printf("TS3TOSSE3: nematodd parameterek: %s - %s \n", pokerName, message);
...
void ts3plugin_onMenuItemEvent(uint64 serverConnectionHandlerID, enum PluginMenuType type, int menuItemID, uint64 selectedItemID) {
	printf("TS3TOSSE3: onMenuItemEvent: serverConnectionHandlerID=%llu, type=%d, menuItemID=%d, selectedItemID=%llu\n", (long long unsigned int)serverConnectionHandlerID, type, menuItemID, (long long unsigned int)selectedItemID);
	switch(type) {
...
void ts3plugin_onHotkeyEvent(const char* keyword) {
	printf("TS3TOSSE3: Hotkey event: %s\n", keyword);
...
/*
	A simple text file reader method from:
...
int sse3_readConfig(char *filename){
	...
	if (address_json != NULL) {
		address = cJSON_GetObjectItemCaseSensitive(address_json, "address");
		if (cJSON_IsString(address) && (address->valuestring != NULL)){
			char *valuestring = address->valuestring;
			strcpy(SSE3HostNameWithPort, valuestring);
			address->valuestring;
			int address_length = strlen(valuestring);
			for (int i = address_length; i >= 0; i--) {
				char c = valuestring[i];
				if (':' == c) {
					str_cut(valuestring, 0, i+1);
					SSE3Port  = atoi(valuestring);
					SSE3HostName = "127.0.0.1"; //Manual assignment since this is always localhost
					printf("TS3TOSSE3: SSE3 hostname found at %s \n", SSE3HostName);
					printf("TS3TOSSE3: SSE3 port found at %d \n", SSE3Port);
					printf("TS3TOSSE3: SSE3 full hostname and port %s \n", SSE3HostNameWithPort);
					break;
				}
...
/*
	Sends the JSON data to the specified endpoint of the SSE3 listener
	
	endpontName - name of the endpoint specified in SSE3_ENDPOINT_% variables
...
int sse3_executePost(char *endpointName, char *jsonStringData){
	...
	if (curl != NULL) {
		...
		char hostWithEndpoint[256] = "";
		strcat(hostWithEndpoint, SSE3HostNameWithPort);
		strcat(hostWithEndpoint, "/");
		strcat(hostWithEndpoint, endpointName);

		printf("TS3TOSSE3: Sending data to endpoint: %s with data:\n %s \n", hostWithEndpoint, jsonStringData);
		...
		curl_easy_setopt(curl, CURLOPT_URL, hostWithEndpoint);

		curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "POST");
		curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
		curl_easy_setopt(curl, CURLOPT_POSTFIELDS, jsonStringData);
		curl_easy_setopt(curl, CURLOPT_USERAGENT, "libcrp/0.1");

		printf("\nTS3TOSSE3: ---- ENDPOINT RESPONSE BEGIN ---- \n");
		res = curl_easy_perform(curl);
		printf("\nTS3TOSSE3: ----  ENDPOINT RESPONSE END  ---- \n");
...
int sse3_registerPlugin() {
	...
	cJSON *pluginDescription = cJSON_CreateString(SSE_PLUGIN_DESCRIPTION);
...
/*
 * Registers the sender portion of the Poke event in teamspeak
 * This event only displays only the sender's name
...
int sse3_registerPluginEventPokeHeader() {
	...
	cJSON *pluginName = cJSON_CreateString(SSE_PLUGIN_NAME);
	cJSON *eventName = cJSON_CreateString("POKE_SENDER");
	cJSON *iconId = cJSON_CreateNumber(0);

	/* Screen*/
	cJSON *handlerArray = cJSON_CreateArray();
	cJSON* pokeEventHandlerValues = cJSON_CreateObject();
	cJSON *handlerDeviceType = cJSON_CreateString("screened");
	cJSON *handlerMode = cJSON_CreateString("screen");
	cJSON *handlerZone = cJSON_CreateString("one");

	cJSON *handlerDatasArray = cJSON_CreateArray();
	cJSON* handlerDatasValuesSender = cJSON_CreateObject();
	cJSON *handlerDatasHasTextSender = cJSON_CreateTrue;
	cJSON *handlerDatasIconIdSender = cJSON_CreateNumber(16);
	cJSON *handlerDatasPrefixSender = cJSON_CreateString("Poke from ");
	/* Tactile event */
	cJSON* pokeEventHandlerTactileValues = cJSON_CreateObject();
	cJSON *pokeEventHandlerTactileDeviceType = cJSON_CreateString("tactile");
	cJSON *pokeEventHandlerTactileMode = cJSON_CreateString("vibrate");
	cJSON *pokeEventHandlerTactileZone = cJSON_CreateString("one");
	...
	cJSON_AddItemToObject(pokeEvent, "game", pluginName);
	cJSON_AddItemToObject(pokeEvent, "event", eventName);
	cJSON_AddItemToObject(pokeEvent, "icon_id", iconId);
	cJSON_AddItemToObject(pokeEvent, "handlers", handlerArray);

	cJSON_AddItemToObject(pokeEventHandlerValues, "device-type", handlerDeviceType);
	cJSON_AddItemToObject(pokeEventHandlerValues, "mode", handlerMode);
	cJSON_AddItemToObject(pokeEventHandlerValues, "zone", handlerZone);
	cJSON_AddItemToObject(pokeEventHandlerValues, "datas", handlerDatasArray);
	cJSON_AddItemToArray(handlerArray, pokeEventHandlerValues);


	cJSON_AddItemToObject(pokeEventHandlerTactileValues, "device-type", pokeEventHandlerTactileDeviceType);
	cJSON_AddItemToObject(pokeEventHandlerTactileValues, "mode", pokeEventHandlerTactileMode);
	cJSON_AddItemToObject(pokeEventHandlerTactileValues, "zone", pokeEventHandlerTactileZone);
	cJSON_AddItemToObject(pokeEventHandlerTactileValues, "pattern", pokeEventHandlerTactilePatternsArray);
	cJSON_AddItemToObject(pokeEventHandlerTactilePatternsArrayValues, "type", pokeEventHandlerTactilePatternsType);

	cJSON_AddItemToArray(handlerArray, pokeEventHandlerTactileValues);
	cJSON_AddItemToArray(pokeEventHandlerTactilePatternsArray, pokeEventHandlerTactilePatternsArrayValues);
...
int sse3_registerPluginEventTextDisplay() {
	...
	cJSON *pluginName = cJSON_CreateString(SSE_PLUGIN_NAME);
	cJSON *eventName = cJSON_CreateString("POKE_MESSAGE");
	cJSON *iconId = cJSON_CreateNumber(0);
	...
	cJSON *handlerDatasArray = cJSON_CreateArray();
	cJSON* handlerDatasValuesMessage = cJSON_CreateObject();
	cJSON *handlerDatasHasTextMessage = cJSON_CreateTrue;
	cJSON *handlerDatasMilisMessage = cJSON_CreateNumber(5000);
	cJSON *handlerDatasArgs = cJSON_CreateString("(custom-text:(context-frame: self))");
	...
	cJSON_AddBoolToObject(handlerDatasValuesMessage, "has-text", handlerDatasHasTextMessage); 
	cJSON_AddItemToObject(handlerDatasValuesMessage, "length-millis", handlerDatasMilisMessage);
	cJSON_AddItemToObject(handlerDatasValuesMessage, "arg", handlerDatasArgs);
...
int sse3_sendEventPokeHeader(char *userName) {
	...
	cJSON *eventPLuginNameSender = cJSON_CreateString(SSE_PLUGIN_NAME);
	cJSON *eventEeventNameSender = cJSON_CreateString("POKE_SENDER");
...
int sse3_sendEventTextDisplay(char *message) {
	...
	cJSON *eventPLuginName = cJSON_CreateString(SSE_PLUGIN_NAME);
	cJSON *eventEeventName = cJSON_CreateString("POKE_MESSAGE");
...
//List of unused events, currently not implemented, but left here for later use
/*
